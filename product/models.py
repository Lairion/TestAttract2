...
class ProductTemplate(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.slug = slugify(self.name)
        productsT = ProductTemplate.objects.filter(
            name=self.name).exclude(id=self.id)
        if len(productsT) > 0:
            self.slug += "-" + str(self.id)
...
class ProductProduct(models.Model):
 ...
 list_product/   """
    sale_order = models.ForeignKey(
        'shop.SaleOrder', related_name="list_product")
    product_template = models.ForeignKey(
        ProductTemplate, related_name="product_product")
    count_product = models.IntegerField(default=1)
    amount_total = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)

    def __str__(self):
        return "%s %s X %s" % (self.product_template.name,
                               self.product_template.price,
                               self.count_product)

    def save(self, *args, **kwargs):
        self.amount_total = self.product_template.price * \
            int(self.count_product)
