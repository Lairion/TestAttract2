...
class My_shop_view:
    ...
    def list_customer(request):
        ...
        for user in users:
            customer = {"name":user}
            print(user.sale_order.all())
            sale_order = user.sale_order.all()[len(user.sale_order.all())-1] if len(user.sale_order.all())>=1 else False
            if sale_order:
                if (sale_order.date_reserve == today or 
                    sale_order.date_order_confirm == today or
                    sale_order.confirmed):
                    print(sale_order.list_product.all())
        ...
        if request.method == "POST":
            ...
            if product_temp:
                ...
                for id_temp in product_temp:
                    ...
                    product_product = list_product.filter(product_template = temp)
                    if len(product_product) > 0:
                        product_product[0].count_product += 1

                        product_product[0].save()
                    else:
                        sale_order.list_product.create(product_template=temp,count_product=1)
